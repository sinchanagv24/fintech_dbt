{{
  config(
    ...
    pre_hook=[
      "
      CREATE OR REPLACE MODEL `{{ target.project }}.{{ target.schema }}.risk_high_risk_cnt_arima_model`
      OPTIONS(
...
WITH actuals AS (
  SELECT
    month,
    CAST(high_risk_customers AS FLOAT64)          AS value,
...
forecast AS (
  SELECT
    ...
    'forecast'                                          AS kind
  FROM ML.FORECAST(
    MODEL `{{ target.project }}.{{ target.schema }}.risk_high_risk_cnt_arima_model`,
